defmodule Blexchain.UsersControllerTest do
  ...
  describe "transfer" do
    test "unable to transfer negtive amount", %{conn: conn} do
      ...
      assert json_response(conn, 200) |> String.starts_with?("Cannot transfer this amount")
    end
    ...
    test "unable to transfer when from user does not exist", %{conn: conn} do
      ...
      assert json_response(conn, 200) |> String.starts_with?("invalid doesnt exist")
    end
    ...
    test "unable to transfer when insufficient funds", %{conn: conn} do
      ...
      assert json_response(conn, 200) |> String.starts_with?("from has insuficient funds")
    end
